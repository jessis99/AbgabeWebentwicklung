...
class Projekte extends BaseController
{
    ...
    public function projekte_edit_auswahl_delete(){
        ...
        if (isset($_POST['btnLoeschen'])){
            echo '
            <script>
                if(confirm("Das Löschen dieses Projektes ist unwiederruflich. Sind Sie sich sicher, dass Sie dieses Projekt löschen wollen?")){
                     window.location.href="'.base_url("Projekte/deleteProjekt?projektauswahl=".$_POST['projektauswahl']).'"; 
                }
            </script>
            ';
           // $data['title'] = 'Projekte';
    ...
    public function deleteProjekt() {
        $data['title'] = 'Projekte';
        $this->ProjekteModel->deleteProjekt($_GET['projektauswahl']);
        session()->set('projekt', NULL);
