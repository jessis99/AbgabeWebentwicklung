...
class Aufgaben extends BaseController
{
    ...
    public function index(){
        if (isset($_POST['btnBearbeiten'])){
            $data['aufgabe']=$this->AufgabenModel->getData($_POST['id'], $_SESSION['projekt']);
            $data['aufgabenmitglieder']=$this->AufgabenModel->getAufgabenMitglieder($_POST['id']);
        }
        if (isset($_POST['btnLoeschen'])){
            ?>
            <script>
                alert("Das Löschen dieser Aufgabe ist unwiederruflich. Sind Sie sich sicher, dass Sie diese Aufgabe löschen wollen?");
            </script>
            <?php
            $data['aufgabe']=$this->AufgabenModel->getData($_POST['id'], $_SESSION['projekt']);
        ...
        $data['aufgaben']= $this->AufgabenModel->getData(null, $_SESSION['projekt']);
